...
User turns whose `content` is a **list of blocks** (not a bare string) — which is how the harness stores any prompt carrying an image or attachment — have their `text` blocks rendered too, so image-bearing prompts are not silently dropped. `image` blocks are embedded inline as a self-contained base64 `data:` URI (`![Image #N](data:image/…;base64,…)`) so they render in any markdown viewer; the pixels come from the transcript's base64 (the `image-cache` PNG the harness references is ephemeral and often already deleted). `[Image #N]` is labeled from `imagePasteIds` to match the in-text marker the harness leaves in the prompt. In `--raw` mode (plain text) the image is replaced with a short `[Image #N: <media_type>]` placeholder instead of the multi-hundred-KB URI. `isMeta` user events (harness-injected command-body expansions and `[Image: source: …]` refs) are suppressed unless `--verbatim` is set.
